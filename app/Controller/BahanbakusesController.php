    ...
    public function index() {
      $this->params['action'] = 'Daftar';
        if ($this->request->is(array('post', 'put'))) {
              ...
              if($reqdata['bahanbakuses']['awal'] == ''){
                $awal = '';
                $tgl_awal = '';
              }else{
                $tgl_awal = $reqdata['bahanbakuses']['awal'];
                @$aw=date_create($reqdata['bahanbakuses']['awal']);
                @$awal=date_format($aw, 'Y-m-d');
              }

              if($reqdata['bahanbakuses']['akhir'] == ''){
                $akhir = '';
                $tgl_akhir = '';
              }else{
                $tgl_akhir = $reqdata['bahanbakuses']['akhir'];
                @$ak=date_create($reqdata['bahanbakuses']['akhir']);
                @$akhir=date_format($ak, 'Y-m-d');
              }

              if($nota != '' && $nama != '' && $awal != ''){
                $where = "WHERE penjualans.nomor LIKE '%$nota%' AND customers.nama LIKE '%$nama%' AND mid(penjualans.created,1,10) BETWEEN '$awal' AND '$akhir'";
              }else if($nota != '' && $nama != '' && $awal == ''){
                $where = "WHERE penjualans.nomor LIKE '%$nota%' AND customers.nama LIKE '%$nama%'";
              }else if($nota != '' && $nama == '' && $awal != ''){
                $where = "WHERE penjualans.nomor LIKE '%$nota%' AND mid(penjualans.created,1,10) BETWEEN '$awal' AND '$akhir'";
              }else if($nota != '' && $nama == '' && $awal == ''){
                $where = "WHERE penjualans.nomor LIKE '%$nota%'";
              }else if($nota == '' && $nama != '' && $awal != ''){
                $where = "WHERE customers.nama LIKE '%$nama%' AND mid(penjualans.created,1,10) BETWEEN '$awal' AND '$akhir'";
              }else if($nota == '' && $nama != '' && $awal == ''){
                $where = "WHERE customers.nama LIKE '%$nama%'";
              }else if($nota == '' && $nama == '' && $awal != ''){
                $where = "WHERE mid(penjualans.created,1,10) BETWEEN '$awal' AND '$akhir'";
              }else if($nota == '' && $nama == '' && $awal == ''){
                $where = "";
              }
              ...
              LEFT JOIN bahanbakus ON penjualans.id = bahanbakus.penjualan_id ".$where." group by penjualans.id");
                          $this->set(compact('bahanbakuses', 'nota','nama','tgl_awal','tgl_akhir'));
        } else {
          $nota = "";
          $nama = "";
          $tgl_awal = "";
...
//            return $this->redirect(array('controller' => 'bahanbakuses', 'action' => 'view/' . $id . '/ygsy'));
    ...
    public function view($id = null) {
      $this->params['action'] = 'Lihat';
        ...
        $disc=$this->Bahanbakus->query("SELECT detail_penjualans.disc, detail_penjualans.hidden_disc, detail_penjualans.id_karyawan, detail_penjualans.ket FROM `detail_penjualans` where penjualan_id='$id' limit 1 ");

        $bayars = $this->Bahanbakus->query("SELECT * from bayars where id_penjualan='$id'");
        //var_dump($bayars);die;
        $this->set(compact('bakus', 'totals', 'id','disc', 'bayars'));
    }

    public function bayar() {
      $this->params['action'] = 'Bayar';
        @$tipe=$_POST['tipe'];
        @$ket=$_POST['ket'];
        @$bayar=$_POST['bayar'];
        @$kbayar=$_POST['kbayar'];
        ...
        @$total=$_POST['total'];
        $bayars = $this->Bahanbakus->query("SELECT * from bayars where id_penjualan='$idpenju'");
        if(count($bayars) == 0)
          ...
          $bayar = $bayar + $paid;
          $this->Bahanbakus->query("update `bayars` set bayar = '$bayar', id_penjualan = '$idpenju', total = '$total', tipe_bayar = '$tipe', ket = '$ket';");
        }
    }
