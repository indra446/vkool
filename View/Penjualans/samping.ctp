...
foreach($_SESSION["cart_samping"] as $k => $item){
	...
	<td><button type='button' class='btn btn-xs btn-danger' id='".$k."' onClick='configsamping(this)'><i class='fa fa-trash-o'></i></button></td></tr>"; 
}$x++;}
}
?>
	</tbody>
...
function configsamping(clicked) {
	var norandom = $("#norandom").val();
	var id = clicked.id;
 	$.ajax({
	type: "POST",
	url: "<?php echo $this -> webroot; ?>Penjualans/del_produksamping/"+norandom,
